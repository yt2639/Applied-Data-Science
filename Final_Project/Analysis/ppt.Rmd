...
dat.autism.child=read.arff("Autism-Child-Data.arff")
dat.autism.adolescent=read.arff("Autism-Adolescent-Data.arff")
dat.autism.adult=read.arff("Autism-Adult-Data.arff")
...
d4 <- data.table(Subject = "percent of jundice = yes",
...
d11 <- data.table(Subject = "percent of jundice = yes",
...
d18 <- data.table(Subject = "percent of jundice = yes",
...
## Our goal
-**Autism Quotient (AQ-50) TAT   $\to$   AQ-10   $\to$   AQ-???**
-**Accuracy & Sensitivity**

## Autism Data
```{r description of data}
description <- data.table(
  Feature = c("Age","Gender","Ethnicity","Born with jaundice","Born with jaundice","Country of residence","Used the screening app before","A1: Question 1 Answer","~A10: Question 10 Answer","Class/ASD"),
...
***
Born with jaundice (Children): There is no significant difference. 
```{r children jaundice}
datatable(d4)
```

***
Country of residence (Children)
```{r children country}
datatable(d5)
```

***
Family member with PDD & Used the screening app before (Children)
```{r pdd & app}
datatable(rbind(d6,d7))
```

***

Age & Gender (Adolescents):
```{r adolescent}
datatable(rbind(d8,d9))
```

***
Ethnicity (Adolescents):
```{r adolescent ethnicity}
datatable(d10)
```

***
Born with jaundice (Adolescents):
```{r adolescent jaundice}
datatable(d11)
```

***
Country of residence (Adolescents)
```{r adolescent country}
datatable(d12)
```

***
Family member with PDD & Used the screening app before (Adolescents)
```{r adolescent pdd & app}
datatable(rbind(d13,d14))
```
...
***
Born with jaundice (Adults):
```{r adult jaundice}
datatable(d18)
```

***
Country of residence (Adults)
```{r adult country}
datatable(d19)
```

***
Family member with PDD & Used the screening app before (Adults)
```{r adult pdd & app}
datatable(rbind(d20,d21))
```
...
```
## Methodology <img src="main.jpg",width = 40%;>
...
ensem.adult.results=knn.fit(dat=dat.ensem.adult,
                              kernel="rectangular",k=4,
...
rownames(four.method.comparison.adult) <- c("knn relief","ridge oneR","tree oneR","ensemble")
